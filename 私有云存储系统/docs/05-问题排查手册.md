# 问题排查手册

完整的故障诊断和问题解决指南。

## 📋 快速诊断流程

```
1. 确认问题现象
   ↓
2. 查看服务状态
   ↓
3. 检查日志文件
   ↓
4. 验证配置文件
   ↓
5. 测试网络连接
   ↓
6. 应用解决方案
   ↓
7. 验证问题解决
```

## 🔍 一、服务无法启动

### 1.1 后端服务无法启动

**现象**：systemctl status显示failed或inactive

**排查步骤**：

```bash
# 1. 查看服务状态
sudo systemctl status raspberrycloud

# 2. 查看详细日志
sudo journalctl -u raspberrycloud -n 50 --no-pager

# 3. 查看应用日志
sudo tail -100 /var/log/raspberrycloud/backend_error.log
```

**常见原因及解决方案**：

#### 原因1：端口被占用

```bash
# 检查8000端口
sudo lsof -i :8000

# 解决方案：杀死占用进程
sudo kill -9 <PID>

# 或修改服务端口
sudo nano /etc/systemd/system/raspberrycloud.service
# 修改 --port 8000 为其他端口
sudo systemctl daemon-reload
sudo systemctl restart raspberrycloud
```

#### 原因2：Python依赖缺失

```bash
# 检查虚拟环境
source /opt/raspberrycloud/venv/bin/activate
python -c "import fastapi"

# 如果报错，重新安装依赖
cd /opt/raspberrycloud
pip install -r requirements.txt
```

#### 原因3：数据库连接失败

```bash
# 检查数据库文件
ls -la /opt/raspberrycloud/backend/raspberrycloud.db

# 如果是MariaDB
sudo systemctl status mariadb
sudo mysql -u clouduser -p -e "SHOW DATABASES;"

# 检查.env配置
sudo nano /opt/raspberrycloud/backend/.env
# 验证DATABASE_URL是否正确
```

#### 原因4：权限问题

```bash
# 修复权限
sudo chown -R www-data:www-data /opt/raspberrycloud
sudo chmod -R 755 /opt/raspberrycloud

# 重启服务
sudo systemctl restart raspberrycloud
```

### 1.2 Nginx无法启动

**现象**：nginx -t测试失败

**排查步骤**：

```bash
# 测试配置
sudo nginx -t

# 查看错误日志
sudo tail -50 /var/log/nginx/error.log
```

**常见原因及解决方案**：

#### 原因1：配置语法错误

```bash
# 详细检查配置
sudo nginx -t 2>&1

# 根据错误提示修改配置文件
sudo nano /etc/nginx/sites-available/raspberrycloud

# 常见错误：
# - 缺少分号 ;
# - 花括号不匹配
# - 指令拼写错误
```

#### 原因2：端口冲突

```bash
# 检查80端口
sudo lsof -i :80

# 如果Apache占用
sudo systemctl stop apache2
sudo systemctl disable apache2
```

#### 原因3：SSL证书路径错误

```bash
# 检查证书文件
sudo ls -la /etc/letsencrypt/live/your-domain.com/

# 如果不存在，注释HTTPS配置或重新申请证书
sudo certbot certonly --nginx -d your-domain.com
```

## 🌐 二、网络访问问题

### 2.1 无法访问Web界面

**现象**：浏览器显示"无法访问此网站"

**排查步骤**：

```bash
# 1. Ping测试
ping 树莓派IP

# 2. 端口测试
telnet 树莓派IP 80
# 或
nc -zv 树莓派IP 80

# 3. 检查防火墙
sudo ufw status

# 4. 检查服务状态
sudo systemctl status nginx
sudo systemctl status raspberrycloud
```

**解决方案**：

```bash
# 开放端口
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp

# 确保服务运行
sudo systemctl start nginx
sudo systemctl start raspberrycloud

# 检查Nginx监听
sudo netstat -tlnp | grep nginx
```

### 2.2 502 Bad Gateway错误

**现象**：Nginx返回502错误

**原因**：后端服务未运行或连接失败

**排查步骤**：

```bash
# 1. 检查后端服务
sudo systemctl status raspberrycloud

# 2. 检查后端监听
curl http://localhost:8000/api/health

# 3. 查看Nginx错误日志
sudo tail -50 /var/log/nginx/raspberrycloud_error.log
```

**解决方案**：

```bash
# 重启后端服务
sudo systemctl restart raspberrycloud

# 等待服务启动
sleep 5

# 重启Nginx
sudo systemctl restart nginx

# 验证
curl -I http://localhost/api/health
```

### 2.3 404 Not Found错误

**现象**：静态文件或API返回404

**原因1：前端文件未正确部署**

```bash
# 检查前端文件
ls -la /var/www/raspberrycloud/

# 重新复制前端文件
sudo cp -r /path/to/frontend/* /var/www/raspberrycloud/
sudo chown -R www-data:www-data /var/www/raspberrycloud
```

**原因2：Nginx location配置错误**

```bash
sudo nano /etc/nginx/sites-available/raspberrycloud
# 检查location块配置
sudo nginx -t
sudo systemctl reload nginx
```

### 2.4 外网无法访问

**现象**：局域网可访问，外网无法访问

**排查步骤**：

```bash
# 1. 验证端口映射
# 在外网用手机4G访问: http://公网IP:映射端口

# 2. 检查DDNS解析
nslookup your-domain.com
# 或
dig your-domain.com

# 3. 测试端口开放
# 使用在线工具: http://tool.chinaz.com/port/
```

**解决方案**：

```
1. 重新配置路由器端口映射
   - 确认内部IP为树莓派静态IP
   - 确认内部端口为80/443
   - 确认外部端口未被占用

2. 更新DDNS
   - 登录DDNS服务商后台
   - 手动更新IP地址
   - 重启DDNS客户端服务

3. 检查运营商限制
   - 某些运营商封禁80/443端口
   - 使用8080/8443等端口
```

## 📁 三、文件操作问题

### 3.1 文件上传失败

**现象**：上传文件时报错或进度卡住

**原因1：文件过大**

```bash
# 检查配置
sudo nano /opt/raspberrycloud/backend/.env
# 查看 MAX_FILE_SIZE 设置

# 检查Nginx限制
sudo nano /etc/nginx/sites-available/raspberrycloud
# 查看 client_max_body_size
```

**原因2：存储空间不足**

```bash
# 检查磁盘空间
df -h /mnt/cloud_storage

# 清理临时文件
sudo rm -rf /mnt/cloud_storage/temp/*

# 检查用户配额
# 通过Web界面或API查看
```

**原因3：权限问题**

```bash
# 修复权限
sudo chown -R www-data:www-data /mnt/cloud_storage
sudo chmod -R 755 /mnt/cloud_storage

# 检查SELinux（如果启用）
sudo setenforce 0  # 临时禁用测试
```

### 3.2 文件下载失败

**现象**：点击下载无响应或下载中断

**排查步骤**：

```bash
# 1. 检查文件是否存在
sudo ls -la /mnt/cloud_storage/users/*/filename

# 2. 检查Nginx超时设置
sudo nano /etc/nginx/sites-available/raspberrycloud
# 查看proxy_read_timeout等设置

# 3. 查看错误日志
sudo tail -50 /var/log/nginx/raspberrycloud_error.log
```

**解决方案**：

```nginx
# 增加超时时间
proxy_read_timeout 3600s;
proxy_send_timeout 3600s;
client_body_timeout 3600s;
```

### 3.3 无法删除文件

**现象**：删除操作无响应或报错

**排查步骤**：

```bash
# 1. 检查文件所有权
sudo ls -la /path/to/file

# 2. 检查进程占用
sudo lsof /path/to/file

# 3. 查看应用日志
sudo tail -50 /var/log/raspberrycloud/backend.log
```

**解决方案**：

```bash
# 强制删除（小心！）
sudo rm -f /path/to/file

# 重启服务
sudo systemctl restart raspberrycloud
```

## 🔐 四、认证问题

### 4.1 无法登录

**现象**：输入正确的用户名密码仍无法登录

**排查步骤**：

```bash
# 1. 检查数据库
sqlite3 /opt/raspberrycloud/backend/raspberrycloud.db "SELECT * FROM users;"

# 2. 检查token生成
sudo tail -50 /var/log/raspberrycloud/backend.log | grep -i auth

# 3. 检查浏览器控制台
# F12 → Console → 查看JavaScript错误
```

**解决方案**：

#### 原因1：密码错误

```bash
# 重置管理员密码
cd /opt/raspberrycloud/backend
source ../venv/bin/activate
python << EOF
from models import SessionLocal, User
db = SessionLocal()
admin = db.query(User).filter(User.username == 'admin').first()
admin.hashed_password = User.hash_password('NewPassword123!')
db.commit()
print("密码已重置为: NewPassword123!")
EOF
```

#### 原因2：数据库损坏

```bash
# 备份数据库
cp /opt/raspberrycloud/backend/raspberrycloud.db /tmp/backup.db

# 检查数据库
sqlite3 /opt/raspberrycloud/backend/raspberrycloud.db "PRAGMA integrity_check;"

# 如果损坏，从备份恢复
# 查看 /mnt/cloud_storage/backups/
```

### 4.2 Token过期或无效

**现象**：已登录但突然要求重新登录

**原因**：

- SECRET_KEY被修改
- 系统时间不正确
- Token已过期

**解决方案**：

```bash
# 检查系统时间
date
# 如果不正确
sudo timedatectl set-ntp true

# 检查.env配置
sudo nano /opt/raspberrycloud/backend/.env
# 确认SECRET_KEY未被修改
# 确认ACCESS_TOKEN_EXPIRE_MINUTES设置合理

# 重启服务
sudo systemctl restart raspberrycloud
```

## 💾 五、数据库问题

### 5.1 数据库锁定

**现象**：操作时提示"database is locked"

**原因**：多个进程同时访问SQLite

**解决方案**：

```bash
# 1. 检查访问数据库的进程
sudo lsof /opt/raspberrycloud/backend/raspberrycloud.db

# 2. 重启服务
sudo systemctl restart raspberrycloud

# 3. 如果频繁发生，考虑升级到MariaDB
```

### 5.2 数据库文件丢失

**现象**：服务启动失败，提示找不到数据库

**解决方案**：

```bash
# 1. 查找数据库
sudo find / -name "raspberrycloud.db" 2>/dev/null

# 2. 从备份恢复
sudo cp /mnt/cloud_storage/backups/database_latest.db.gz /tmp/
sudo gunzip /tmp/database_latest.db.gz
sudo cp /tmp/database_latest.db /opt/raspberrycloud/backend/raspberrycloud.db
sudo chown www-data:www-data /opt/raspberrycloud/backend/raspberrycloud.db

# 3. 如果无备份，重新初始化
cd /opt/raspberrycloud/backend
source ../venv/bin/activate
python -c "from models import init_db; init_db()"
```

## 🗂️ 六、存储问题

### 6.1 外接硬盘未挂载

**现象**：启动后/mnt/cloud_storage为空

**排查步骤**：

```bash
# 1. 检查挂载状态
mountpoint /mnt/cloud_storage
df -h | grep cloud_storage

# 2. 查看设备
lsblk
sudo fdisk -l

# 3. 检查fstab
cat /etc/fstab | grep cloud_storage

# 4. 查看系统日志
dmesg | grep -i usb
dmesg | grep -i sda
```

**解决方案**：

```bash
# 手动挂载
sudo mount -a

# 如果失败，检查设备
sudo blkid
# 更新fstab中的UUID

# 重新运行挂载脚本
sudo bash /opt/raspberrycloud/scripts/mount_storage.sh
```

### 6.2 存储空间不足

**现象**：上传失败，提示空间不足

**排查步骤**：

```bash
# 1. 检查磁盘使用
df -h /mnt/cloud_storage

# 2. 查看大文件
sudo du -sh /mnt/cloud_storage/*
sudo find /mnt/cloud_storage -type f -size +1G -exec ls -lh {} \;

# 3. 检查用户使用情况
sudo du -sh /mnt/cloud_storage/users/*
```

**解决方案**：

```bash
# 1. 清理临时文件
sudo rm -rf /mnt/cloud_storage/temp/*

# 2. 清理旧备份
sudo rm -f /mnt/cloud_storage/backups/*_$(date -d '30 days ago' +%Y%m%d)*.gz

# 3. 删除已标记删除的文件（需要开发清理脚本）
# 或扩容存储设备
```

## ⚙️ 七、性能问题

### 7.1 系统运行缓慢

**排查步骤**：

```bash
# 1. 查看系统资源
htop
# 或
top

# 2. 检查CPU温度
vcgencmd measure_temp

# 3. 查看内存使用
free -h

# 4. 检查磁盘I/O
iostat -x 1

# 5. 查看网络流量
iftop  # 需要安装: sudo apt install iftop
```

**解决方案**：

#### 原因1：CPU过热

```bash
# 检查温度
vcgencmd measure_temp

# 如果>80°C
# - 添加散热片/风扇
# - 改善通风
# - 降低CPU频率（不推荐）
```

#### 原因2：内存不足

```bash
# 增加SWAP
sudo nano /etc/dphys-swapfile
# 修改 CONF_SWAPSIZE=2048
sudo /etc/init.d/dphys-swapfile restart

# 重启服务以释放内存
sudo systemctl restart raspberrycloud
```

#### 原因3：磁盘I/O瓶颈

```bash
# 检查SD卡速度
sudo hdparm -t /dev/mmcblk0

# 建议：
# - 使用SSD作为系统盘
# - 将数据存储移到USB 3.0硬盘
# - 启用Nginx缓存
```

### 7.2 上传/下载速度慢

**排查步骤**：

```bash
# 1. 测试局域网速度
# 使用iperf3
# 服务端（树莓派）
iperf3 -s

# 客户端（电脑）
iperf3 -c 树莓派IP

# 2. 检查网络连接
ethtool eth0  # 有线网络
iwconfig wlan0  # 无线网络
```

**解决方案**：

```
1. 使用有线网络（千兆网线+千兆路由器）
2. 减少网络干扰（WiFi信道优化）
3. 升级路由器固件
4. 启用Nginx gzip压缩
5. 优化MTU值
```

## 🛠️ 八、常用命令

### 服务管理

```bash
# 查看状态
sudo systemctl status raspberrycloud
sudo systemctl status nginx

# 启动/停止/重启
sudo systemctl start raspberrycloud
sudo systemctl stop raspberrycloud
sudo systemctl restart raspberrycloud

# 开机自启
sudo systemctl enable raspberrycloud
sudo systemctl disable raspberrycloud

# 查看日志
sudo journalctl -u raspberrycloud -f
sudo journalctl -u raspberrycloud --since "10 minutes ago"
sudo journalctl -u raspberrycloud --since today
```

### 日志查看

```bash
# 应用日志
sudo tail -f /var/log/raspberrycloud/backend.log
sudo tail -f /var/log/raspberrycloud/backend_error.log

# Nginx日志
sudo tail -f /var/log/nginx/raspberrycloud_access.log
sudo tail -f /var/log/nginx/raspberrycloud_error.log

# 系统日志
sudo tail -f /var/log/syslog
sudo tail -f /var/log/auth.log
```

### 数据库操作

```bash
# SQLite
sqlite3 /opt/raspberrycloud/backend/raspberrycloud.db

# 常用SQL命令
.tables  # 列出所有表
.schema users  # 查看表结构
SELECT * FROM users;  # 查询用户
.exit  # 退出

# MariaDB
sudo mysql -u clouduser -p
SHOW DATABASES;
USE raspberrycloud;
SHOW TABLES;
SELECT * FROM users;
EXIT;
```

### 网络诊断

```bash
# 测试连接
ping 树莓派IP
telnet 树莓派IP 80
curl -I http://localhost/api/health

# 查看端口
sudo netstat -tlnp
sudo lsof -i :8000

# 查看路由
ip route

# DNS查询
nslookup your-domain.com
dig your-domain.com
```

## 📞 九、获取帮助

### 9.1 收集诊断信息

运行诊断脚本：

```bash
# 创建诊断脚本
cat > /tmp/diagnose.sh << 'EOF'
#!/bin/bash
echo "=== RaspberryCloud诊断信息 ==="
echo "时间: $(date)"
echo ""

echo "=== 系统信息 ==="
uname -a
cat /etc/os-release | grep PRETTY_NAME
echo ""

echo "=== 服务状态 ==="
systemctl status raspberrycloud --no-pager
systemctl status nginx --no-pager
echo ""

echo "=== 磁盘使用 ==="
df -h
echo ""

echo "=== 内存使用 ==="
free -h
echo ""

echo "=== CPU温度 ==="
vcgencmd measure_temp
echo ""

echo "=== 最近错误日志 ==="
sudo tail -30 /var/log/raspberrycloud/backend_error.log
sudo tail -30 /var/log/nginx/raspberrycloud_error.log
EOF

bash /tmp/diagnose.sh > /tmp/diagnose.txt
cat /tmp/diagnose.txt
```

### 9.2 社区支持

- GitHub Issues: <项目仓库地址>
- 树莓派论坛: https://forums.raspberrypi.com/
- FastAPI文档: https://fastapi.tiangolo.com/

### 9.3 提交问题时包含

1. 问题详细描述
2. 重现步骤
3. 预期结果 vs 实际结果
4. 诊断信息（diagnose.txt）
5. 相关日志
6. 系统配置

## ✅ 快速参考

| 问题 | 快速解决 |
|------|---------|
| 服务无法启动 | `sudo systemctl restart raspberrycloud` |
| 502错误 | 检查后端服务是否运行 |
| 404错误 | 检查前端文件和Nginx配置 |
| 无法登录 | 重置密码或检查数据库 |
| 上传失败 | 检查磁盘空间和权限 |
| 外网无法访问 | 检查端口映射和DDNS |
| 系统缓慢 | 检查CPU温度和内存 |

## 🎉 总结

遇到问题不要慌，按照以下步骤排查：

1. **确认现象** - 准确描述问题
2. **查看日志** - 日志是最好的老师
3. **逐步排查** - 从简单到复杂
4. **验证修复** - 确保问题真正解决
5. **记录经验** - 避免重复问题

大部分问题都可以通过查看日志和重启服务解决！

