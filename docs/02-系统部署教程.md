...
如果需要重新部署，请先清理之前的部署内容：

```bash
# 方法1：如果项目目录还存在，运行清理脚本
sudo bash /opt/raspberrycloud/scripts/cleanup.sh

# 方法2：如果项目目录已删除，从Git仓库运行
...
**注意**：清理脚本会保留用户数据（`/mnt/cloud_storage`），如需完全清理请手动删除。

## 🗂️ 步骤2：下载项目代码

### 2.1 选择部署方案

**方案A：直接在部署目录使用Git（适合已部署的情况）**

如果你的项目已经部署在 `/opt/raspberrycloud`：
...
**方案B：开发目录 + 部署目录分离（推荐，最佳实践）⭐**

推荐使用独立的开发目录，然后部署到 `/opt/raspberrycloud`：
...
**如果项目已在 `/opt/raspberrycloud`：**

```bash
# 方法1：直接在部署目录更新（需要修复权限）
cd /opt/raspberrycloud
...
**详细更新流程请参考：**
- [Windows开发-树莓派部署流程](Windows开发-树莓派部署流程.md)
- [从GitHub更新代码](从GitHub更新代码.md)
