# RaspberryCloud 项目运行指南

完整的项目运行方法，包括开发环境和生产环境。

## 🚀 快速开始

### 在树莓派上运行（生产环境）

#### 方法一：使用systemd服务（推荐，开机自启）

```bash
# 1. 启动服务
sudo systemctl start raspberrycloud

# 2. 检查状态
sudo systemctl status raspberrycloud

# 3. 查看日志
sudo journalctl -u raspberrycloud -f

# 4. 访问
# 浏览器打开: http://树莓派IP
```

#### 方法二：手动启动（测试/调试用）

```bash
# 1. 进入后端目录
cd /opt/raspberrycloud/backend

# 2. 激活虚拟环境
source /opt/raspberrycloud/venv/bin/activate

# 3. 启动服务
uvicorn main:app --host 0.0.0.0 --port 8000 --reload

# 4. 访问
# 后端API: http://树莓派IP:8000
# Web界面: http://树莓派IP (通过Nginx)
```

#### 方法三：使用启动脚本

```bash
# 使用快速启动脚本
cd ~/Desktop/Github/RaspiOwnCloud
chmod +x scripts/start.sh
sudo bash scripts/start.sh
```

### 在本地Windows上运行（开发环境）

#### 方法一：使用启动脚本（推荐）

```powershell
# 在项目根目录运行
.\scripts\start.bat
```

#### 方法二：手动启动

```powershell
# 1. 进入后端目录
cd backend

# 2. 创建虚拟环境（如果还没有）
python -m venv venv

# 3. 激活虚拟环境
.\venv\Scripts\activate

# 4. 安装依赖
pip install -r requirements.txt

# 5. 创建.env配置文件
copy ..\config\env.example .env
# 编辑.env文件，修改存储路径等配置

# 6. 初始化数据库
python -c "from models import init_db; init_db()"

# 7. 启动服务
uvicorn main:app --host 0.0.0.0 --port 8000 --reload
```

## 📋 详细运行步骤

### 生产环境运行（树莓派）

#### 前置条件检查

```bash
# 1. 检查服务是否已安装
sudo systemctl status raspberrycloud

# 2. 检查Nginx是否运行
sudo systemctl status nginx

# 3. 检查端口是否被占用
sudo lsof -i :8000
sudo lsof -i :80
```

#### 启动服务

```bash
# 启动后端服务
sudo systemctl start raspberrycloud

# 启动Nginx（如果未运行）
sudo systemctl start nginx

# 设置开机自启
sudo systemctl enable raspberrycloud
sudo systemctl enable nginx
```

#### 验证运行

```bash
# 1. 检查服务状态
sudo systemctl status raspberrycloud
sudo systemctl status nginx

# 2. 测试后端API
curl http://localhost:8000/api/health
# 应该返回: {"status":"healthy","version":"1.0.0"}

# 3. 测试通过Nginx访问
curl http://localhost/api/health

# 4. 获取访问地址
hostname -I
# 使用返回的IP访问: http://树莓派IP
```

#### 访问Web界面

1. **局域网访问**
   - 浏览器打开：`http://树莓派IP`
   - 例如：`http://192.168.1.100`

2. **登录**
   - 用户名：`admin`
   - 密码：`RaspberryCloud2024!`
   - ⚠️ 首次登录后立即修改密码

### 开发环境运行（本地Windows）

#### 环境准备

```powershell
# 1. 检查Python版本（需要3.11+）
python --version

# 2. 进入项目目录
cd F:\Github\RaspiOwnCloud\backend

# 3. 创建虚拟环境
python -m venv venv

# 4. 激活虚拟环境
.\venv\Scripts\activate
```

#### 安装依赖

```powershell
# 升级pip
python -m pip install --upgrade pip

# 安装依赖
pip install -r requirements.txt
```

#### 配置环境变量

```powershell
# 1. 复制配置模板
copy ..\config\env.example .env

# 2. 编辑配置文件
notepad .env
```

**Windows环境配置示例：**

```bash
# 数据库配置（SQLite）
DATABASE_URL=sqlite:///./raspberrycloud.db

# 存储路径配置（Windows格式）
STORAGE_PATH=C:\temp\raspberrycloud\storage\users
SHARE_PATH=C:\temp\raspberrycloud\storage\shares
TEMP_PATH=C:\temp\raspberrycloud\storage\temp
BACKUP_PATH=C:\temp\raspberrycloud\storage\backups

# 生成密钥
SECRET_KEY=你的随机密钥（运行: openssl rand -hex 32）
```

#### 初始化数据库

```powershell
# 初始化数据库
python -c "from models import init_db; init_db()"
```

#### 启动服务

```powershell
# 方法1：直接运行
uvicorn main:app --host 0.0.0.0 --port 8000 --reload

# 方法2：使用Python运行
python main.py
```

#### 访问服务

- **后端API**: `http://localhost:8000`
- **API文档**: `http://localhost:8000/api/docs`（如果DEBUG=true）
- **前端界面**: 需要单独启动前端服务器或使用Nginx

## 🔧 运行模式说明

### 开发模式（带热重载）

```bash
# 代码修改后自动重启
uvicorn main:app --host 0.0.0.0 --port 8000 --reload
```

### 生产模式（多进程）

```bash
# 使用多个worker进程
uvicorn main:app --host 0.0.0.0 --port 8000 --workers 2
```

### 调试模式（详细日志）

```bash
# 启用DEBUG模式
# 在.env中设置: DEBUG=true
uvicorn main:app --host 0.0.0.0 --port 8000 --log-level debug
```

## 📊 服务管理命令

### systemd服务管理（树莓派）

```bash
# 启动服务
sudo systemctl start raspberrycloud

# 停止服务
sudo systemctl stop raspberrycloud

# 重启服务
sudo systemctl restart raspberrycloud

# 查看状态
sudo systemctl status raspberrycloud

# 查看实时日志
sudo journalctl -u raspberrycloud -f

# 查看最近50条日志
sudo journalctl -u raspberrycloud -n 50

# 设置开机自启
sudo systemctl enable raspberrycloud

# 取消开机自启
sudo systemctl disable raspberrycloud
```

### 手动运行管理

```bash
# 启动（前台运行，可以看到日志）
cd /opt/raspberrycloud/backend
source /opt/raspberrycloud/venv/bin/activate
uvicorn main:app --host 0.0.0.0 --port 8000

# 后台运行
nohup uvicorn main:app --host 0.0.0.0 --port 8000 > /tmp/raspberrycloud.log 2>&1 &

# 停止后台进程
ps aux | grep uvicorn
kill <PID>
```

## 🐛 常见问题

### Q1: 服务无法启动

```bash
# 查看详细错误
sudo journalctl -u raspberrycloud -n 50 --no-pager

# 手动启动查看错误
cd /opt/raspberrycloud/backend
source /opt/raspberrycloud/venv/bin/activate
uvicorn main:app --host 0.0.0.0 --port 8000
```

### Q2: 端口被占用

```bash
# 查看占用8000端口的进程
sudo lsof -i :8000

# 杀死进程
sudo kill -9 <PID>
```

### Q3: 数据库连接失败

```bash
# 检查数据库文件
ls -la /opt/raspberrycloud/raspberrycloud.db

# 检查.env配置
cat /opt/raspberrycloud/.env | grep DATABASE_URL
```

### Q4: 权限问题

```bash
# 修复权限
sudo chown -R www-data:www-data /opt/raspberrycloud
sudo chmod -R 755 /opt/raspberrycloud
```

## ✅ 运行检查清单

- [ ] Python虚拟环境已创建
- [ ] 依赖已安装（pip install -r requirements.txt）
- [ ] .env配置文件已创建并配置
- [ ] 数据库已初始化
- [ ] 存储目录已创建
- [ ] 服务可以启动
- [ ] API可以访问（/api/health）
- [ ] Web界面可以访问
- [ ] 可以登录

## 🎯 快速验证

运行以下命令验证项目是否正常运行：

```bash
# 1. 检查服务状态
sudo systemctl status raspberrycloud

# 2. 测试API
curl http://localhost:8000/api/health

# 3. 测试登录
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"RaspberryCloud2024!"}'

# 4. 访问Web界面
# 浏览器打开: http://树莓派IP
```

## 📚 相关文档

- [系统部署教程](02-系统部署教程.md) - 完整的部署步骤
- [手动部署命令](手动部署命令.md) - 手动部署的详细命令
- [问题排查手册](05-问题排查手册.md) - 常见问题解决方案




