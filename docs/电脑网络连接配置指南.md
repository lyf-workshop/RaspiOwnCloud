# 电脑网络连接配置指南

当树莓派通过电脑网络连接时的外网访问配置方案。

## 🔍 网络拓扑分析

### 当前情况
```
互联网
  ↓
路由器 (公网IP)
  ↓
电脑 (192.168.x.x)
  ↓
树莓派 (通过电脑网络连接)
```

### 问题
- 树莓派没有直接连接到路由器
- 无法在路由器上直接配置端口转发到树莓派
- 需要通过电脑进行转发

## 🚀 解决方案

### 方案一：将树莓派直接连接到路由器（推荐）

这是最简单可靠的方案。

#### 步骤1：连接方式

**有线连接：**
```
1. 用网线连接树莓派和路由器
2. 树莓派会自动获取IP地址
3. 在路由器管理页面查看树莓派的IP
```

**无线连接：**
```bash
# 在树莓派上配置WiFi
sudo raspi-config
# 选择: System Options → Wireless LAN
# 输入WiFi名称和密码
```

#### 步骤2：获取树莓派IP

```bash
# 在树莓派上执行
hostname -I

# 或查看路由器管理页面
# 找到设备名为 "raspberrypi" 的设备
```

#### 步骤3：配置路由器端口转发

现在可以正常配置路由器端口转发了。

---

### 方案二：使用电脑作为转发（复杂，不推荐）

如果必须通过电脑网络连接，需要配置电脑的端口转发。

#### Windows系统

**使用netsh配置端口转发：**

```powershell
# 以管理员身份运行PowerShell

# 1. 启用IP转发
netsh interface portproxy add v4tov4 listenport=8080 listenaddress=0.0.0.0 connectport=80 connectaddress=树莓派IP

# 2. 添加防火墙规则
New-NetFirewallRule -DisplayName "RaspberryCloud HTTP" -Direction Inbound -LocalPort 8080 -Protocol TCP -Action Allow

# 3. 查看规则
netsh interface portproxy show all

# 4. 删除规则（如果需要）
netsh interface portproxy delete v4tov4 listenport=8080 listenaddress=0.0.0.0
```

**配置路由器：**
- 外部端口：8080 → 内部IP：电脑IP → 内部端口：8080

#### Linux/Mac系统

**使用iptables配置端口转发：**

```bash
# 1. 启用IP转发
sudo sysctl -w net.ipv4.ip_forward=1

# 2. 配置iptables规则
sudo iptables -t nat -A PREROUTING -p tcp --dport 8080 -j DNAT --to-destination 树莓派IP:80
sudo iptables -t nat -A POSTROUTING -j MASQUERADE

# 3. 保存规则（Ubuntu/Debian）
sudo apt install iptables-persistent
sudo netfilter-persistent save
```

---

### 方案三：使用内网穿透（最简单，推荐）

如果无法改变网络结构，使用内网穿透工具。

#### 选项1：使用frp（免费，自建服务器）

**需要：**
- 一台有公网IP的服务器（或使用云服务器）

**配置步骤：**

1. **在服务器上安装frp服务端**
```bash
# 下载frp
wget https://github.com/fatedier/frp/releases/download/v0.52.3/frp_0.52.3_linux_arm64.tar.gz
tar -xzf frp_0.52.3_linux_arm64.tar.gz
cd frp_0.52.3_linux_arm64

# 配置服务端
nano frps.ini
```

服务端配置：
```ini
[common]
bind_port = 7000
vhost_http_port = 8080
vhost_https_port = 8443
```

2. **在树莓派上安装frp客户端**
```bash
# 下载frp
wget https://github.com/fatedier/frp/releases/download/v0.52.3/frp_0.52.3_linux_arm64.tar.gz
tar -xzf frp_0.52.3_linux_arm64.tar.gz
cd frp_0.52.3_linux_arm64

# 配置客户端
nano frpc.ini
```

客户端配置：
```ini
[common]
server_addr = 你的服务器IP
server_port = 7000

[web]
type = http
local_port = 80
custom_domains = 你的域名.com
```

#### 选项2：使用ngrok（免费，无需服务器）

**注册账号：**
1. 访问 https://ngrok.com
2. 注册免费账号
3. 获取authtoken

**在树莓派上安装：**
```bash
# 下载ngrok
wget https://bin.equinox.io/c/bNyj1mQVY4c/ngrok-v3-stable-linux-arm64.tgz
tar -xzf ngrok-v3-stable-linux-arm64.tgz
sudo mv ngrok /usr/local/bin/

# 配置authtoken
ngrok config add-authtoken 你的token

# 启动隧道
ngrok http 80
```

**访问：**
- ngrok会提供一个临时域名，如：`https://abc123.ngrok.io`
- 免费版域名每次启动会变化

#### 选项3：使用Cloudflare Tunnel（免费，推荐）

**优势：**
- 完全免费
- 可以使用自己的域名
- 不需要公网IP

**配置步骤：**

1. **注册Cloudflare账号**
   - 访问 https://www.cloudflare.com
   - 注册账号并添加你的域名

2. **在树莓派上安装cloudflared**
```bash
# 下载安装
wget https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-arm64
sudo mv cloudflared-linux-arm64 /usr/local/bin/cloudflared
sudo chmod +x /usr/local/bin/cloudflared

# 登录
cloudflared tunnel login

# 创建隧道
cloudflared tunnel create raspberrycloud

# 配置路由
cloudflared tunnel route dns raspberrycloud 你的域名.com

# 运行隧道
cloudflared tunnel run raspberrycloud
```

---

### 方案四：使用ZeroTier（虚拟局域网）

创建虚拟局域网，让树莓派获得虚拟IP。

**配置步骤：**

1. **注册ZeroTier**
   - 访问 https://www.zerotier.com
   - 注册免费账号

2. **创建网络**
   - 登录后创建网络
   - 记录Network ID

3. **在树莓派上安装**
```bash
# 安装
curl -s https://install.zerotier.com | sudo bash

# 加入网络
sudo zerotier-cli join 你的NetworkID

# 在ZeroTier网站批准设备
```

4. **配置**
   - 树莓派会获得一个虚拟IP（如：10.147.x.x）
   - 在任何地方都可以通过这个IP访问

---

## 🎯 推荐方案对比

| 方案 | 难度 | 成本 | 稳定性 | 推荐度 |
|------|------|------|--------|--------|
| 直接连路由器 | ⭐ 简单 | 免费 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| Cloudflare Tunnel | ⭐⭐ 中等 | 免费 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| ZeroTier | ⭐⭐ 中等 | 免费 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| frp | ⭐⭐⭐ 复杂 | 需要服务器 | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| ngrok | ⭐ 简单 | 免费（有限制） | ⭐⭐⭐ | ⭐⭐⭐ |
| 电脑转发 | ⭐⭐⭐ 复杂 | 免费 | ⭐⭐ | ⭐⭐ |

## 🚀 快速开始（推荐：直接连路由器）

### 步骤1：连接树莓派到路由器

**有线连接：**
```
1. 用网线连接树莓派和路由器
2. 等待自动获取IP
```

**无线连接：**
```bash
# 在树莓派上
sudo raspi-config
# System Options → Wireless LAN
# 输入WiFi名称和密码
```

### 步骤2：获取树莓派IP

```bash
# 在树莓派上
hostname -I

# 或查看路由器管理页面
# 设备列表中找到 "raspberrypi"
```

### 步骤3：配置路由器端口转发

现在可以正常配置了：
- 外部端口：80 → 内部IP：树莓派IP → 内部端口：80
- 外部端口：443 → 内部IP：树莓派IP → 内部端口：443

### 步骤4：继续域名配置

```bash
# 运行域名配置脚本
cd ~/Desktop/Github/RaspiOwnCloud
sudo bash scripts/setup_aliyun_domain.sh
```

## 💡 如果无法直接连接路由器

如果确实无法将树莓派直接连接到路由器，推荐使用 **Cloudflare Tunnel** 方案，它：
- 完全免费
- 可以使用自己的域名
- 不需要公网IP
- 不需要端口转发

需要我帮你配置 Cloudflare Tunnel 吗？






