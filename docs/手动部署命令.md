# RaspberryCloud 手动部署命令（完整版）

适用于项目根目录为 `RaspiOwnCloud` 的情况。

## 📋 前置条件

- 树莓派已安装 Raspberry Pi OS (Bookworm) 64位
- 已通过 SSH 连接到树莓派
- 项目文件已上传到树莓派（例如：`/home/pi/RaspiOwnCloud`）
...
1. 访问Web界面并登录
2. 修改默认管理员密码
3. 配置HTTPS和外网访问（参考：`docs/03-多端访问配置.md`）
4. 安全加固（参考：`docs/04-安全加固指南.md`）
